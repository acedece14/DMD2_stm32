...
void DMDFrame::selectFont(const unsigned char * font)
{
  this->font = (uint8_t *)font;
}

unsigned int DMDFrame::drawChar(const int x, const int y, const unsigned char letter, DMDGraphicsMode mode, const uint8_t *font)
{	 
  ...
  if (header.size == 0) {
    ...
    for (uint8_t i = 0; i < c; i++) {
      index += font[sizeof(FontHeader) + i];
    }
    index = index * bytes + header.charCount + sizeof(FontHeader);
    width = font[sizeof(FontHeader) + c];
  }
...
inline void _drawString(DMDFrame *dmd, int x, int y, String str, DMDGraphicsMode mode, const uint8_t *font)
{
  struct FontHeader header;
...
int DMDFrame::charWidth(const unsigned char letter, const uint8_t *font)
{
  struct FontHeader header;
  memcpy(&header, (void*)this->font, sizeof(FontHeader));
